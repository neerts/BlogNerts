...
function openArticleBySlug(slug) {
  ...
  stopAllIframes();
    ...
    const thumb = a.youtubeUrl ? getYouTubeThumb(a.youtubeUrl) : (a.image || "img/nerts-mark.png");
    // PERBAIKAN BUG: Tambahkan (a.description || "") untuk mencegah error jika deskripsi kosong
...
function loadDatabase() {
  // Ambil data dari file JSON, tambahkan cache-busting
  fetch('articles.json?v=' + Date.now())
    ...
    .then(data => {
      // Data berhasil diambil (sudah dalam format array)
      if (Array.isArray(data)) {
        
        // 1. Siapkan semua artikel
        articles = data.map(a => ({
          ...a,
          slug: a.slug || slugify(a.title)
        }));

        // 2. Filter berdasarkan tanggal terbaru
        ...
        throw new Error("Data articles.json bukan array.");
      }
    })
    .catch(error => {
      console.error(error);
      homepage.innerHTML = `<div class="empty" style="padding:20px; text-align:center; color: var(--fg-muted);">${error.message}. Silakan refresh.</div>`;
    });
}
