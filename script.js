...
function loadDatabase() {
  try {
    if (typeof defaultArticles !== 'undefined' && Array.isArray(defaultArticles)) {
      ...
      throw new Error("Variabel 'defaultArticles' tidak ditemukan.");
    }
  } catch (error) {
    console.error(error);
    homepage.innerHTML = `<div class="empty" style="padding:20px; text-align:center; color: var(--fg-muted);">${error.message}. Pastikan articles.js dimuat dengan benar.</div>`;
  }
}
